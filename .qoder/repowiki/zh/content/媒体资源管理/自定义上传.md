...
```mermaid
graph TB
    CustomUploadForm["CustomUploadForm.vue<br/>━━━━━━━━━━<br/>• code: Ref<br/>• editor: EditorView<br/>• formCustomTextarea<br/>• isDark: boolean<br/>━━━━━━━━━━<br/>• formCustomSave()<br/>• initializeEditor()<br/>• setupDarkModeWatch()"]
    
    StorageManager["存储管理器<br/>━━━━━━━━━━<br/>• reactive(key, default)<br/>• get(key): Promise<br/>• set(key, value): Promise"]
    
    EditorView["CodeMirror EditorView<br/>━━━━━━━━━━<br/>• dispatch()<br/>• destroy()"]
    
    CodeMirror["代码编辑器"]
    
    CustomUploadForm -->|"使用"| StorageManager
    CustomUploadForm -->|"实例化"| EditorView
    CustomUploadForm -->|"渲染"| CodeMirror
    
    style CustomUploadForm fill:#e1f5ff,stroke:#0284c7,stroke-width:2px
    style StorageManager fill:#fef3c7,stroke:#f59e0b,stroke-width:2px
    style EditorView fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style CodeMirror fill:#f3e8ff,stroke:#9333ea,stroke-width:2px
```
...
graph TB
    subgraph "接口层"
        StorageEngine["存储引擎接口<br/>━━━━━━━━━━<br/>• get(key)<br/>• set(key, value)<br/>• remove(key)<br/>• has(key)<br/>• clear()<br/>• keys()"]
    end
    
    subgraph "实现层"
        LocalStorage["本地存储引擎<br/>━━━━━━━━━━<br/>LocalStorageEngine<br/>实现基本存储操作"]
        
        RestfulStorage["RESTful 存储引擎<br/>━━━━━━━━━━<br/>RestfulStorageEngine<br/>实现 HTTP 请求<br/>• request(method, endpoint)"]
    end
    
    subgraph "管理层"
        StorageManager["存储管理器<br/>━━━━━━━━━━<br/>• setEngine(engine)<br/>• getEngine()<br/>• get(key)<br/>• set(key, value)<br/>• getJSON(key, default)<br/>• setJSON(key, value)<br/>• reactive(key, default)<br/>• customReactive(key, default)"]
    end
    ...
    LocalStorage -.->|"实现"| StorageEngine
    RestfulStorage -.->|"实现"| StorageEngine
    StorageManager -->|"依赖"| StorageEngine
    StorageManager -->|"默认使用"| LocalStorage
    CustomUploadForm -->|"使用"| StorageManager
    
    style StorageEngine fill:#f3e8ff,stroke:#9333ea,stroke-width:2px
    style LocalStorage fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style RestfulStorage fill:#dbeafe,stroke:#3b82f6,stroke-width:2px
    style StorageManager fill:#fef3c7,stroke:#f59e0b,stroke-width:2px
    style CustomUploadForm fill:#e1f5ff,stroke:#0284c7,stroke-width:2px
```
